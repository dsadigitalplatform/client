...
import { createAssociateType } from '@features/associate-types/services/associateTypesService'
...
const AssociateTypesCreateForm = ({
  onSuccess,
  onCancel,
  showTitle = true,
  initialValues,
  ...
  const validate = () => {
    ...
    if (name.trim().length < 2) next.name = 'Associate type is required'
    setFieldErrors(next)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (onSubmitOverride) {
        ...
        await createAssociateType(payload)
      }

      setSuccessMsg(initialValues ? 'Associate type updated successfully' : 'Associate type created successfully')
      ...
      if (onSuccess) onSuccess()
      if (!onSuccess && !onSubmitOverride) router.push('/associate-types')
    } catch (e: any) {
      setError(e?.message || 'Failed to save associate type')
  ...
  return (
    <Card sx={{ borderRadius: 3, boxShadow: 'var(--mui-customShadows-lg, 0px 6px 24px rgba(0,0,0,0.08))' }}>
      {showTitle ? <CardHeader title='Add Associate Type' subheader='Define an associate type and its description' /> : null}
      ...
      <CardActions sx={{ px: { xs: 2.5, sm: 3 }, pb: { xs: 2.5, sm: 3 } }}>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2, width: '100%' }}>
          <Button variant='contained' disabled={submitting} onClick={handleSubmit} fullWidth={isMobile}>
            {submitting ? 'Saving...' : submitLabel || 'Save Associate Type'}
          </Button>
          <Button
            variant='outlined'
            disabled={submitting}
            onClick={() => (onCancel ? onCancel() : router.push('/associate-types'))}
            fullWidth={isMobile}
          >
            Cancel
          </Button>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Typography variant='body2' color='text.secondary'>
                  Taking you back to Associate Types...
                </Typography>
              </Box>
            </Box>
...
export default AssociateTypesCreateForm
