  ...
  try {
    if (patch.name != null) {
      ...
      const existing = await db.collection('loanTypes').findOne(
        { tenantId: tenantIdObj, name: { $regex: `^${safeName}$`, $options: 'i' }, _id: { $ne: new ObjectId(id) }, ...userScopedFilter },
        { projection: { _id: 1 } }
      )
...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const res = await db.collection('loanTypes').deleteOne({ _id: loanTypeId, tenantId: tenantIdObj, ...userScopedFilter })
