...
export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const row = await db.collection('associateTypes').findOne({ _id: new ObjectId(id), tenantId: tenantIdObj })
...
export async function PUT(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (patch.name != null && !isNonEmptyString(patch.name)) errors.name = 'Associate type is required'
  ...
  try {
    ...
    if (err && err.code === 11000) {
      return NextResponse.json({ error: 'duplicate_name', message: 'Associate type already exists for this tenant' }, { status: 409 })
    }
...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { db, tenantIdObj, userId, role, isSuperAdmin } = context
  const associateTypeId = new ObjectId(id)
  ...
  const res = await db.collection('associateTypes').deleteOne({ _id: associateTypeId, tenantId: tenantIdObj, ...userScopedFilter })
