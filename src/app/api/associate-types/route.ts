...
export async function GET(request: Request) {
  ...
  const associateTypes = rows.map(r => ({
  ...
  return NextResponse.json({ associateTypes })
}
...
export async function POST(request: Request) {
  ...
  if (!isNonEmptyString(name)) errors.name = 'Associate type is required'
  ...
  try {
    const res = await db.collection('associateTypes').insertOne(doc)
