  ...
  MuiChip: {
    styleOverrides: {
      root: ({ ownerState, theme }) => {
        const colorKey = ownerState.color && ownerState.color !== 'default' ? ownerState.color : null

        return {
          ...theme.typography.body2,
          fontWeight: theme.typography.fontWeightMedium,
          color: colorKey ? `var(--mui-palette-${colorKey}-main)` : 'var(--mui-palette-text-primary)',
          backgroundColor: 'transparent',
          backgroundImage: colorKey
            ? `linear-gradient(135deg, rgb(var(--mui-palette-${colorKey}-mainChannel) / 0.18), rgb(var(--mui-palette-${colorKey}-mainChannel) / 0.04))`
            : 'linear-gradient(135deg, rgb(var(--mui-palette-background-paperChannel) / 0.7), rgb(var(--mui-palette-background-paperChannel) / 0.4))',
          border: '1px solid',
          borderColor: colorKey ? `rgb(var(--mui-palette-${colorKey}-mainChannel) / 0.35)` : 'var(--mui-palette-divider)',
          backdropFilter: 'blur(10px)',
          boxShadow: '0 10px 24px rgb(0 0 0 / 0.1)',
